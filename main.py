...
class MPHandsCompat:
    def __init__(self, max_num_hands=2, min_detection_confidence=0.5, min_tracking_confidence=0.5, model_complexity=0):
    ...
    def process(self, image):
        ...
        if result.hand_landmarks:
            for i, hand_landmarks in enumerate(result.hand_landmarks):
                ...
                hand.landmark = [Landmark(l.x, l.y, l.z) for l in hand_landmarks]
...
def draw_app_icon(surface, app_name, x, y, base_w, base_h, is_selected=False, zoom_scale=1.0, gui_scale=1.0):
    ...
    icon_img = _get_font(icon_size).render(app_name[0], True, (255, 255, 255, 180))
    ...
    text_img = _get_font(text_size).render(app_name, True, (255, 255, 255))
...
def draw_wheel(surface, state, window_width, window_height):
    ...
    white = (255, 255, 255)
    # --- Performance: use a small local alpha surface instead of full-screen ---
    margin = r + int(80 * scale)
    local_w = margin * 2
    local_h = margin * 2
    local_surf = pygame.Surface((local_w, local_h), pygame.SRCALPHA)
    lcx, lcy = margin, margin  # local center
    for i in range(5):
    ...
    font = _get_font(max(18, int(40 * scale)))
...
def carousel_main():
    ...
    _cam_surface = pygame.Surface((320, 240))
    ...
    while running:
        ...
        # Camera preview — only update every 3 frames for performance
        if _cam_frame_count % 3 == 0:
            small_rgb = cv2.resize(rgb, (320, 240))
            pygame.surfarray.blit_array(_cam_surface, np.transpose(small_rgb, (1, 0, 2)))
        screen.blit(_cam_surface, (WINDOW_WIDTH - 330, 10))
...
def email_fallback_main():
    ...
    while running:
        ...
        ok_now = detect_ok_gesture(right_hand, ok_touch_threshold) if right_hand else False
        if ok_now and not ok_prev:
            print("A-OK — closing inbox fallback.")
            running = False
        ok_prev = ok_now

        if right_hand and not wheel_active:
            pinch_now = is_pinching(right_hand, pinch_threshold)
            pos = get_pinch_position(right_hand)
        ...
        status = f"Zoom {zoom_level:.2f}x • Scroll {int(-smooth_scroll_y)}/{max(0, page_height - (WINDOW_HEIGHT - 80))}"
